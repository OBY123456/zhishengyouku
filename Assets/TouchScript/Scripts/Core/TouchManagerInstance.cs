...
namespace TouchScript.Core
{
    ...
    public sealed class TouchManagerInstance : DebuggableMonoBehaviour, ITouchManager
    {
        ...
        private static int pointersLimitCount = 50;
        ...
        private void updateAdded(List<Pointer> pointers)
        {
            ...
            var addedCount = pointers.Count > pointersLimitCount ? pointersLimitCount : pointers.Count;

            var list = pointerListPool.Get();
            for (var i = 0; i < addedCount; i++)
            {
                var pointer = pointers[i];
                if (idToPointer.ContainsKey(pointer.Id))
                {
                  continue;
                }
                list.Add(pointer);
